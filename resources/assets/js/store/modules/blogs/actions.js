...
export const store = ({}, form) => {
    return Vue.request(form).post('/api/blogs', 'blogs/add').then(() => {
...
export const update = ({}, form) => {
    return Vue.request(form).post('/api/blogs/' + form.blog, 'blogs/update').then(() => {
...
export const destroy = ({}, blog) => {
    return Vue.request(blog).delete('/api/blogs/' + blog, 'blogs/remove').then(() => {
